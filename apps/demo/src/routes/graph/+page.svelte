<script lang="ts">
  ...
  // Highlight participants + downstream dependencies
  const highlightedNodes = $derived.by(() => {
    ...
    const direct = new SvelteSet(useCase.participants.map((p) => p.service));
    const all = new SvelteSet(direct);
    for (const edge of data.layout.edges) {
      if (direct.has(edge.source)) all.add(edge.target);
    }
    return [...all];
  });
...
<div class="flex h-[calc(100vh-4rem)] flex-col">
  <div class="shrink-0 space-y-4 p-4 pb-0">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Service Graph</h1>
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
          {data.layout.nodes.length} services, {data.layout.edges.length} connections from {data
            .useCases.length}
          use cases. Click a node to view details.
        </p>
      </div>
  ...
  <div class="min-h-0 flex-1 p-4">
    <div
      class="h-full rounded-lg border border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900"
    >
      <ServiceGraph layout={data.layout} {highlightedNodes} lockZoom />
    </div>
  </div>
</div>
